...
export function updateNovel(id: string, partial: Partial<Omit<Novel, 'id' | 'createdAt'>>): Novel | null {
  const all = getNovels()
  const idx = all.findIndex((n) => n.id === id)
  if (idx < 0) return null
  all[idx] = { ...all[idx], ...partial, updatedAt: nowIso() }
...
export function hydrateNovelWorkspaceFromPayload(novelId: string, payload: WorkspaceSnapshotPayload): void {
  ...
  replaceNovelScopedRows(SCENES_KEY, id, Array.isArray(payload.scenes) ? payload.scenes : [])
  replaceNovelScopedRows(
...
export function deleteNovel(novelId: string, options?: { trackDeletion?: boolean }): boolean {
  ...
  removeNovelScopedRows<Scene>(SCENES_KEY, id)
...
export function buildNovelWorkspacePayload(novelId: string): WorkspaceSnapshotPayload {
  return {
    ...
    scenes: getScenesByNovelId(novelId),
...
export function createCharacter(input: NewCharacterInput, opts?: { autoGenerated?: boolean }): Character {
  ...
  const item: Character = {
    ...
    ...(opts?.autoGenerated ? { autoGenerated: true } : {}),
    createdAt: now,
    updatedAt: now,
  }
  all.push(item)
...
export function createFaction(input: NewFactionInput, opts?: { autoGenerated?: boolean }): Faction {
...
export function createItem(input: NewItemInput, opts?: { autoGenerated?: boolean }): Item {
  const all = getAllItems()
  const now = nowIso()
  const attributes = normalizeItemAttributes(input.attributes)
...
function getAllScenes(): Scene[] {
  const raw = readScopedStorageItem(SCENES_KEY)
  if (!raw) return []
  try {
    const parsed = JSON.parse(raw) as unknown[]
    if (!Array.isArray(parsed)) return []
    return parsed
      ...
      .map((row) => ({
        ...
        views: Array.isArray(row.views) ? (row.views as Scene['views']) : undefined,
        stylePrompt: row.stylePrompt ? String(row.stylePrompt) : undefined,
        locked: row.locked === true ? true : undefined,
        autoGenerated: row.autoGenerated === true ? true : undefined,
        createdAt: String(row.createdAt ?? '') || nowIso(),
        updatedAt: String(row.updatedAt ?? '') || nowIso(),
      }))
      .filter((s) => s.id && s.novelId)
  } catch {
    return []
  }
}

function saveAllScenes(scenes: Scene[]): void {
  writeScopedStorageItem(SCENES_KEY, JSON.stringify(scenes))
  emitStorageChange()
}

export function getScenesByNovelId(novelId: string): Scene[] {
  return getAllScenes()
    .filter((s) => s.novelId === novelId)
    .sort((a, b) => (a.createdAt < b.createdAt ? -1 : 1))
}

export function createScene(input: NewSceneInput, opts?: { autoGenerated?: boolean }): Scene {
  ...
  const scene: Scene = {
    ...
    description: String(input.description ?? '').trim(),
    stylePrompt: input.stylePrompt ? String(input.stylePrompt).trim() : undefined,
    ...(opts?.autoGenerated ? { autoGenerated: true } : {}),
    createdAt: now,
    updatedAt: now,
  }
...
export function updateScene(partial: Pick<Scene, 'id'> & Partial<Scene>): Scene | null {
  const all = getAllScenes()
  ...
  const updated: Scene = {
    ...merged,
    name: String(merged.name ?? '').trim() || all[idx].name || '未命名场景',
...
export function createTimelineEvent(input: NewTimelineEventInput, opts?: { autoGenerated?: boolean }): TimelineEvent {
...
export function createForeshadowPlant(input: NewForeshadowPlantInput, opts?: { autoGenerated?: boolean }): ForeshadowPlant {
...
export type ConfirmableModule = 'characters' | 'factions' | 'items' | 'foreshadows' | 'timeline'

export function confirmEntity(module: ConfirmableModule, id: string): boolean {
  switch (module) {
    case 'characters': {
      const all = getAllCharacters()
      const idx = all.findIndex((c) => c.id === id)
      if (idx < 0) return false
      delete (all[idx] as any).autoGenerated
